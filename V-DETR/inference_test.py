...
def run_tta_inference_with_nms(model, full_pcd_path, voxel_size=20.0, overlap=0.5,
                               use_color=True, num_points=100000, iou_threshold=0.1, conf_threshold=0.1):
    ...
    for x in x_starts:
        for y in y_starts:
            ...
            centers, sizes, yaws, classes, objectness_scores = process_predictions(outputs,threshold=conf_threshold)
...
def save_submission_txt(centers, sizes, yaws, classes, out_path="track1.txt",
    ...
    with open(out_path, 'w') as f:
        for idx, (c, s, y, cls_id) in enumerate(zip(centers, sizes, yaws, classes)):
            c = c.cpu().numpy()
            s = s.cpu().numpy()
            y = y.item()
            cls_id = cls_id.item()
...
def main():
    ...
    centers, sizes, yaws, classes,scores = run_tta_inference_with_nms(
        model,
        '/perception/dataset/PhysicalAI-SmartSpaces/pcd_dataset/test/pcd/Warehouse_019_00000.ply',
        voxel_size=20.0,
        overlap=0.5,
    ...
    print(f"centers: {centers.shape}, sizes: {sizes.shape}, yaws: {yaws.shape}, classes: {classes.shape}")
    save_predictions_to_ply(centers, sizes, yaws, out_path="test.ply")
    save_submission_txt(centers, sizes, yaws, classes, out_path='test.txt',
                                scene_id=0, frame_id=0)
