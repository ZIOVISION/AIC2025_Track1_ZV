...
def eval_det_cls(
    pred, gt, ovthresh=0.25, use_07_metric=False, get_iou_func=get_iou_obb, size=''
):
    ...
    for img_id in gt.keys():
        ...
        if len(bbox) > 0 and size != '':
            box3d_vol = box3d_vol_batch(bbox)
            if size == 'S':
                filt = box3d_vol < area_rng[0]
            elif size == 'M':
                filt = np.logical_and(box3d_vol > area_rng[0], box3d_vol < area_rng[1])
            elif size == 'L':
                filt = box3d_vol > area_rng[1]
            bbox = bbox[filt]
    ...
    for img_id in pred.keys():
        for box, score in pred[img_id]:
            ...
            box3d_vol = box3d_vol_batch(box[None,:,:])
            if size == 'S':
                filt = box3d_vol < area_rng[0]
            elif size == 'M':
                filt = np.logical_and(box3d_vol > area_rng[0], box3d_vol < area_rng[1])
...
def eval_det_cls_wrapper(arguments):
    pred, gt, ovthresh, use_07_metric, get_iou_func, size = arguments
    rec, prec, ap = eval_det_cls(pred, gt, ovthresh, use_07_metric, get_iou_func, size)
    return (rec, prec, ap)
...
def eval_det_multiprocessing(
    pred_all, gt_all, ovthresh=0.25, use_07_metric=False, get_iou_func=get_iou_obb, size=''
):
    """Generic functions to compute precision/recall for object detection
    for multiple classes.
    Input:
    ...
    ret_values = p.map(
        ...
        [
            (pred[classname], gt[classname], ovthresh, use_07_metric, get_iou_func, size)
            for classname in gt.keys()
